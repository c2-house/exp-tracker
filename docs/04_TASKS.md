...
- [ ] **화면 이동 설정 (Expo Router)**
  - `app/index.tsx`의 플로팅 액션 버튼(FAB)에 Link 컴포넌트를 적용하여, 새 상품 등록 화면(예: `app/add-product.tsx`)으로 이동하도록 설정합니다.
- [ ] **새 화면 생성: app/add-product.tsx (화면 2: 카메라)**
  - 와이어프레임(v1.3)의 화면 2: 카메라 촬영 화면 UI(가이드라인, 컨트롤 버튼 등)를 구현합니다.
- [ ] **카메라 기능 구현**
  - `expo-camera` 라이브러리를 설치하고, `app/add-product.tsx`에서 사용자 권한을 요청하고 카메라 뷰가 나오도록 구현합니다.
- [ ] **이미지 처리 (압축)**
  - `expo-image-manipulator` 라이브러리를 설치합니다.
  - 카메라 촬영 또는 갤러리 선택 직후, 이미지를 가로 500px 정도로 리사이징하고 압축하여 용량을 최소화하는 로직을 구현합니다.
- [ ] **OCR 기능 구현 (핵심)**
  - `react-native-mlkit` 라이브러리를 설치하고 설정합니다.
  - 촬영 버튼을 눌렀을 때 사진을 찍고, (압축된 이미지를 바탕으로) ML Kit의 `recognizeText` 함수로 텍스트를 추출하는 로직을 구현합니다.
  - **로딩 인디케이터 구현 (화면 2.5)**: 텍스트 추출 중임을 알리는 로딩 스크린을 표시합니다.
- [ ] **새 화면 생성: `app/confirm-product.tsx` (화면 3: 결과 확인)**
  - `app/add-product.tsx`에서 OCR 결과(상품명, 유통기한) 및 **압축된 이미지 URI**를 `expo-router`의 `params`로 넘겨받아 화면 3: OCR 결과 확인 및 수정 화면을 구현합니다.
  - Date Picker, 카테고리 선택 UI 등을 구현합니다.
- [ ] **2단계 촬영 로직 구현**
  - 화면 3에서 유통기한 인식이 실패했을 때(`params`가 비어있을 때), '유통기한 촬영' 버튼이 나타나고, 이 버튼을 누르면 다시 `app/add-product.tsx` 화면으로 이동하도록 구현합니다.
- [ ] **'등록하기' 로직 구현**
  - 화면 3에서 '등록하기' 버튼을 누르면, 새로운 상품 객체(압축된 이미지 URI 포함)를 생성하여 `sampleItems` 배열(추후 로컬 저장소)에 추가하고, 메인 화면(`app/index.tsx`)으로 복귀하도록 구현합니다.
...
## 6단계: 로컬 저장 및 프리미엄 백업 (Local Storage & Premium Backup)
...
- [ ] **로컬 저장소 설정 (AsyncStorage)**
  - `@react-native-async-storage/async-storage` 라이브러리를 설치합니다.
- [ ] **로컬 C-R-U-D 로직 구현**
  - 현재 `sampleItems` 배열(메모리)을 사용하고 있는 모든 데이터 처리 로직(등록, 수정, 삭제, 사용 완료, 조회)이 `AsyncStorage`에 JSON 문자열 형태로 데이터를 읽고 쓰는 로직으로 전면 수정합니다.
  - 앱 시작 시 `AsyncStorage`에서 데이터를 비동기적으로 불러와 `sampleItems` 상태를 초기화합니다.
- [ ] **'설정'** 화면 및 '백업' 버튼 UI 생성
