...
## 화면 흐름 1: 상품 등록 (Form 기반)

**사용자 목표:** 상품 등록 폼을 통해 제품을 등록하며, 유통기한은 OCR로 간편하게 입력한다. (PRD User Story 1.1)
...
4. 상품 추가 버튼 (Floating Action Button)
   ...
   - **동작:** 탭하면 **'화면 2: 상품 등록 폼'**으로 이동한다.

### 화면 2: 상품 등록 폼 (Add Product Form)

사용자가 신규 상품의 정보를 입력하는 핵심 화면.
...
2. 상품명 필드 (Product Name) - Required

   - `TextInput`

3. 유통기한 필드 (Expiry Date) - Required

   - **날짜 표시 영역:** "유통기한을 입력하세요" 또는 선택된 날짜(예: 2025.12.31) 표시. 탭하면 Date Input 열림. (숫자 키 형태, 달력 아님)
   - **OCR 버튼:** 날짜 표시 영역 우측에 '카메라' 아이콘 배치.
   - **동작:** '카메라' 아이콘 탭 시 **'화면 2.5: 유통기한 OCR 스캐너'**로 이동.

4. 카테고리 선택 (Category) - Required

   - 원형 아이콘 버튼 중에서 하나만 선택

5. 푸터 (Footer)

   - '저장하기' 버튼 (모든 필수 필드 입력 시 활성화)

### 화면 2.5: 유통기한 OCR 스캐너 (Expiry Date Scanner)
...
2. 가이드라인 오버레이 (Guideline Overlay)
   ...
   - 가이드라인 위에 "상품명이 잘 보이게 촬영해주세요"와 같은 텍스트를 표시하여 사용자 행동을 유도한다.
...
3. 하단 컨트롤 바 (Bottom Control Bar)
   ...
   - **동작:**
     - 사진 촬영/선택 시, 카메라 뷰 대신 촬영/선택된 사진을 전체화면 배경으로 보여준다.
     - 앱이 이미지에서 텍스트를 분석하고 추출하는 동안 로딩 화면을 잠시 표시한다.
     - 로딩 중에는 화면 중앙 가이드라인 위에 가로선이 위아래로 움직이는 애니메이션을 표시하여 사진을 스캔하는 듯한 효과를 내고, 그 아래에 "유통기한 인식 중..."과 같은 텍스트를 표시한다.
     - 텍스트 인식(`react-native-mlkit`) 완료 후, 날짜 데이터를 `params`에 담아 '화면 2'로 자동 복귀(`router.back`)한다.

4. 상단 컨트롤 바 (Top Control Bar)
   - (좌측) **닫기 아이콘 (Close Icon 'X'):** 촬영을 취소하고 '화면 1: 메인 목록 화면'으로 돌아간다.

### 화면 3: 등록 완료 후 메인 목록 (Updated Main List Screen)
...
### 화면 4: 상품 상세 화면 (Product Detail Screen)
...
### 화면 5: 알림 목록 화면 (Notification List Screen)
